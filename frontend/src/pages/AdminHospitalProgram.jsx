  ...
  return (
    <div className="border border-ink/10 rounded-card p-5">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {c.status === "pending_discharge" && (
            <span className="text-xs font-semibold px-3 py-1 rounded-full bg-amber-100 text-amber-700">
              Awaiting {c.hospital_discharge_at && !c.officer_discharge_at ? "officer's" : "hospital's"} discharge confirmation
            </span>
          )}
        </div>
      </div>
        ...
        <div className="text-xs text-ink/50 mb-3 flex flex-wrap items-center gap-2">
          ...
          {dischargeLink && (
            <button type="button" onClick={copyLink} className="font-semibold text-violet hover:underline">
              {copied ? "Copied!" : "Copy officer's discharge-confirmation link"}
            </button>
          )}
          <span className="text-ink/40">
            · Hospital confirmed: {c.hospital_discharge_at ? new Date(c.hospital_discharge_at).toLocaleString() : "no"}
      ...
      <p className="text-[10px] text-ink/40 mt-2">
        This just seeds the assignment history for that stretch — the officer stays on the case and billing keeps
        running day by day until both the hospital and this officer confirm the actual discharge date & time.
      </p>
