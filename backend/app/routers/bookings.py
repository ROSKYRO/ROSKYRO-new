...
def mark_en_route(booking_id: int, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    if booking.status != BookingStatus.assigned:
        raise HTTPException(
            status_code=400,
            detail="Booking must be in 'assigned' status (agent assigned, not yet en route) to do this",
        )
...
def _require_assigned(db: Session, booking_id: int) -> Booking:
    """Fetch the booking and make sure a real Agent is actually attached to it.

    This is the gate for every step after 'requested' (en-route, arrived, start,
    end) - none of them should be reachable on a booking nobody has been sent
    out to, no matter what BookingStatus value it happens to carry.
    """
    booking = db.query(Booking).get(booking_id)
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    if not booking.agent_id:
        raise HTTPException(status_code=400, detail="This booking has no agent assigned yet - assign one first")
    return booking
...
def mark_arrived(booking_id: int, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    if booking.status != BookingStatus.en_route:
        raise HTTPException(
            status_code=400,
            detail="Booking must be 'en_route' before it can be marked arrived",
        )
