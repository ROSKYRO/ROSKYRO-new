...
def assign_officer(
    ...
    coverage or billing stops. The officer picked here becomes the case's
    one assigned Relationship Officer (PatientCase.assigned_agent_id): they
    stay on the hook, and time keeps being counted, day by day, until the
    patient is actually discharged — which needs this same officer's
    confirmation (via their own no-login link) together with the hospital's.
    Re-running this for a date that already has an assignment just swaps the
    officer for that day, so fixing a mistake or handling a no-show is a
    single click, not a new row."""
